steps = {	
	...
	"passo_0210": '{programa_fastq-join} -p 64 -m 200 {F.fastq} {R.fastq} -o {merged.fastq}',
	"passo_0220": '{script_fastq_join} {unpared1.fastq} {unpared2.fastq} -unir* {merged.fastq} -o {final.fastq}',
	"passo_0330": '{script_fasuniq-join} {final.fastq} -o {final_uniq.fastq} {repetition_list.json} ',
	"passo_0400": '{programa_HS-BLASTn} -db {refseq.fna} -window_masker_db {refseq.fna.counts.obinary} -query {final_uniq.fastq} -out {results.fa} -outfmt 7',
	...
	"passo_0710": '{cmd_mkdir} ',
	"passo_0720": '{programa_TransDecoder.LongOrfs} -t {Trinity.fasta}  -m 100',
	"passo_0731": '{programa_makeblastdb}-dbtype 'prot' -in {Trinity.fasta.transdecoder_dir/longest_orfs.pep} -parse_seqids -out {Predicted}',
	"passo_0732": '{programa_blastdbcmd}-db {Predicted} -entry_batch {complete_candidates} | sed -e "s/lcl|//g" > {complete.fasta}',
	...
	"passo_07e1": '{cmd_cat} cat {complete.blastp.m8} | cut -f1 | sort -u > {evidences.list}',
	"passo_07e2": '{programa_makeblastdb}-dbtype 'nucl' -in {Trinity.fasta.transdecoder_dir/longest_orfs.cds} -parse_seqids -out {cds}',
	"passo_07c3": '{programa_blastdbcmd}-db {cds} -entry_batch {evidences.list} | sed -e "s/lcl|//g" > {train.fasta}',
	...
	"passo_0810": '{cmd_mkdir} {reference}',
	...
	"passo_0900": '{programa_samtools} view {STARMapAligned.sortedByCoord.out.bam} | cut -f1,3 > ReadsId.txt #Retirar apenas os Reads Ids',
